...
3. SELECCIÓN DE MERCADO ÓPTIMO: Elige estrictamente SOLO UNO de los siguientes mercados usando EXACTAMENTE el texto del diccionario a continuación (El texto en 'selected_market' DEBE ESTAR ESTRICTAMENTE EN INGLÉS Y EXACTAMENTE COMO APARECE AQUÍ):
   - "Over 2.5 Goals": Solo usar si la cuota está entre 1.65 y 2.30. (Prohibido Over 3.5 u otros).
...
7. REGLA DE PICK PRINCIPAL:
   - La cuota mínima para cualquier pick oficial SIEMPRE debe ser MAYOR O IGUAL a 1.65.
   - Si el payload indica `is_top_match = True`, elige la opción más segura que cumpla esta cuota mínima.
   - Si `is_top_match = False` (Partido APEX), prioriza buscar aún más valor matemático siempre respetando la cuota mínima de 1.65.
